...
and 959 (FTP). Properties can be set both from the command line and
in a configuration file. When the same property is set in both
locations, the command line takes precedence.
...
siege will generate a new config directory when it runs. You can generate
your configuration directory with the following command: siege.config
